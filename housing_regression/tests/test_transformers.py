...
class TestGeneralProperties:
    ...
    TRANSFORMERS = [
        tran.ColumnTransformerDF(
            [("scaler", StandardScaler(), ["num1"])], remainder="passthrough"
        ),
        tran.UnivariateTransformer(variables=["num2"], func=np.log),
        tran.BivariateTransformer(variables=["num1"], reference_var="num2", func=diff),
...
class TestColumnTransformerDF:
    ...
    def transformer(self):
        return tran.ColumnTransformerDF(
            [("sc", StandardScaler(), ["num1"])], remainder="passthrough"
        )
...
class TestUnivariateTransformer:
    ...
    def test_valid_functions(self, func, data):
        ...
        transformer = tran.UnivariateTransformer(variables=["num1", "num2"], func=func)
        transformed = transformer.fit_transform(data)
    ...
    def test_invalid(self, data):
        ...
        transformer = tran.UnivariateTransformer(
            variables=["num1", "num2"], func=math.log
        )
...
class TestBivariateTransformer:
    ...
    def test_valid_functions(self, func, data):
        """Can the transformer apply different functions?"""
        transformer = tran.BivariateTransformer(
            variables=["num1"], reference_var="num2", func=func
        )
        transformed = transformer.fit_transform(data)
